...
try:
    ...
    print("Unable to import BCBio! Please make sure bcbiogff is installed. Error:", e, file=sys.stderr)
    sys.exit(-1)

def star_mapping(index_dir, SR_fofn, mapping_dir, cpus):
    with open(SR_fofn) as fofn:
        for line in fofn:
            files = [x.strip() for x in line.split(' ')]
            sample_name, ext = os.path.splitext(os.path.basename(files[0]))
            compressed = ext == '.gz'
            if compressed:
            ...
            if not os.path.exists(sample_prefix + 'Log.final.out'):
                ...
                if compressed:
                    STAR_args.extend(['--readFilesCommand', 'zcat'])
                print('Mapping for', sample_name, 'in progress...')
                subprocess.call(STAR_args)
                print('Mapping for', sample_name, 'done.')
            else:
                print('Mapping for', sample_name, 'already done. Skipping...')


def star(genome, SR_fofn, output_dir, cpus):
    fasta_genome = genome
    index_dir = os.path.join(output_dir, 'STAR_index')
    index_dir_tmp = os.path.join(index_dir, '_STARtmp')
    index_dir_o = os.path.join(index_dir, 'SAindex')
    mapping_dir = os.path.join(output_dir, 'STAR_mapping')
    ...
    star_mapping(index_dir, SR_fofn, mapping_dir, cpus)
    return (mapping_dir, index_dir)

def kallisto_quantification(files, index, cpus, output_dir):
    r1 = files[0].strip()
    r2 = files[1].strip()
    if os.path.splitext(r1)[1] == '.gz':
        sample_name = os.path.splitext(os.path.basename(r1))[0]
        sample_name = sample_name.split('.')[0]
    else:
        sample_name = os.path.splitext(os.path.basename(r1))[0]
    out_prefix = os.path.join(output_dir, sample_name)
    abundance_file = os.path.join(out_prefix, 'abundance.tsv')
...
def kallisto(corrected_fasta, SR_fofn, output_dir, cpus):
    kallisto_output = os.path.join(output_dir, 'kallisto_output')
    kallisto_index = os.path.join(kallisto_output, 'kallisto_corrected_fasta.idx')
    expression_files = []
    ...
    with open(SR_fofn) as fofn:
        for line in fofn:
            ...
            if len(files) == 2:
                ...
                sys.exit()
            expression_files.append(abundance)
    return ",".join(expression_files)

def get_TSS_bed(corrected_gtf, chr_order):
    limit_info = dict(gff_type=["transcript"])
    out_directory = os.path.dirname(corrected_gtf)
    tmp_in = os.path.join(out_directory, 'coverage_inside_TSS.bed_tmp')
    tmp_out = os.path.join(out_directory, 'coverage_outside_TSS.bed_tmp')
    ...
    inside_sorted = os.path.join(out_directory, "inside_TSS.bed")
    outside_sorted = os.path.join(out_directory, "outside_TSS.bed")
...
def get_bam_header(bam):
    ...
    out = os.path.join(o_dir, "chr_order.txt")
...
## get ratios across replicates and return it as a dictionary
    ...
    out_TSS_file = os.path.join(os.path.dirname(inside_bed), "ratio_TSS.csv")
    in_bed = pybedtools.BedTool(inside_bed)
    out_bed = pybedtools.BedTool(outside_bed)
    for b in list(range(0, len(replicates))):
